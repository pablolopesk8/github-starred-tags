...
describe('Validator GetStarred Test', () => {
    ...
    it('Should be rejected if user is not a string', async () => {
        const data = { user: 456 };
        await getStarredValidator(data).should.be.rejectedWith('invalid-user');
    });

    it('Should be rejected if has tag and it is not string', async () => {
        const data = { user: 'any123', tags: [ 123 ]};
        await getStarredValidator(data).should.be.rejectedWith('invalid-tag');
    });

    it('Should be accept if has an user', async () => {
        const data = { user: 'any123' };
        const result = await getStarredValidator(data);
        result.should.be.true();
    });

    it('Should be accept if has an user and array of string tags', async () => {
